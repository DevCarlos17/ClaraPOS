...
export function DashboardKpiCards() {
  ...
  const tasaDesactualizada = tasa
    ? isTasaDesactualizada(tasa.created_at)
    : false;
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
      ...
      <KpiCard
        ...
        subtitulo={
          loadingVentas || loadingTasa
            ? "..."
            : formatBs(usdToBs(totalVentasUsd, tasaValor))
        }
        icon={ShoppingCart}
        color="blue"
      />
      ...
      <KpiCard
        ...
        subtitleClassName={
          tasaDesactualizada ? "text-red-600 font-medium" : undefined
        }
      />
      ...
      <KpiCard
        ...
        subtitulo={
          loadingCxc || loadingTasa
            ? "..."
            : formatBs(usdToBs(totalCxcUsd, tasaValor))
        }
        icon={CreditCard}
        color="red"
      />
...
function KpiCard({
  ...
  const colorMap: Record<string, { iconColor: string }> = {
    blue: { iconColor: "text-blue-600" },
    green: { iconColor: "text-green-600" },
    amber: { iconColor: "text-amber-500" },
    red: { iconColor: "text-red-600" },
    purple: { iconColor: "text-purple-600" },
  };
  ...
  return (
    <div className="rounded-2xl bg-card shadow-lg overflow-hidden">
      <div className="relative bg-primary px-6 pt-5 pb-7">
        <p className="text-sm font-semibold text-primary-foreground/90 leading-tight">
          {titulo}
        </p>
      ...
      <div className="px-6 pt-9 pb-5">
        ...
        <p
          className={`text-xs mt-1.5 ${subtitleClassName ?? "text-muted-foreground"}`}
        >
          {subtitulo}
        </p>
      </div>
