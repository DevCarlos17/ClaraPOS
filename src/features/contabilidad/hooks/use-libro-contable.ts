...
import { generarAsientos, reversarAsientos } from '@/features/contabilidad/lib/generar-asientos'
...
export async function conciliarAsiento(id: string): Promise<void> {
  await db.writeTransaction(async (tx) => {
    ...
    if (asiento.estado !== 'PENDIENTE') {
      throw new Error(`El asiento no esta en estado PENDIENTE (estado actual: ${asiento.estado})`)
    }
...
export async function reversarAsientoManual(
  ...
  await db.writeTransaction(async (tx) => {
    ...
    const asiento = result.rows.item(0) as { estado: string }
    if (asiento.estado !== 'PENDIENTE') {
      throw new Error(`Solo se pueden reversar asientos PENDIENTES (estado actual: ${asiento.estado})`)
    }
