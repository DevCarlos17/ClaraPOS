...
import { useDetalleCxcDia } from '../hooks/use-cuadre'
...
export function CxcModal({ isOpen, onClose, fecha }: CxcModalProps) {
  const dialogRef = useRef<HTMLDialogElement>(null)
  const { facturas, isLoading } = useDetalleCxcDia(fecha)
        ...
        <div className="flex items-start justify-between mb-4 shrink-0">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {fecha} &middot; {facturas.length} factura(s) con saldo pendiente
            </p>
          </div>
