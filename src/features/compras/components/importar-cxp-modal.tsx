...
import {
  importarSaldosInicialesCxp,
  generarPlantillaCxpCsv,
} from '../hooks/use-importar-cxp'
import {
  UploadSimple,
  FileArrowDown,
  CheckCircle,
...
export function ImportarCxpModal({ isOpen, onClose }: Props) {
  ...
  function handleDescargarPlantilla() {
    const csv = generarPlantillaCxpCsv()
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url
    a.download = 'cxp_plantilla_importacion.csv'
    a.click()
  ...
  function procesarCsv(raw: string) {
    const { rows: rawRows, parseErrors } = parseCsv<Record<string, string>>(raw, CXP_CSV_HEADER_MAP)
            ...
            <div className="space-y-5">
              ...
              <div className="rounded-lg border border-border p-4 space-y-3 bg-muted/30">
                ...
                <p className="text-xs text-muted-foreground">
                  ...
                  su propio numero de factura (SIP-000001, SIP-000002...).
                </p>
              </div>
