...
export function CajaList() {
  ...
  const [editingCaja, setEditingCaja] = useState<Caja | undefined>(undefined)
  ...
  function handleEditar(caja: Caja) {
    setEditingCaja(caja)
    setFormOpen(true)
  }
  ...
  async function handleToggleActivo(caja: Caja) {
    const nuevoEstado = caja.is_active !== 1
    setTogglingId(caja.id)
    try {
      await actualizarCaja(caja.id, { is_active: nuevoEstado })
      toast.success(nuevoEstado ? 'Caja activada' : 'Caja desactivada')
    } catch (error) {
    ...
    <div>
      ...
      {cajas.length === 0 ? (
        <div className="text-center py-12 text-gray-500">
          <p className="text-base font-medium">No hay cajas registradas</p>
          <p className="text-sm mt-1">Crea la primera caja para comenzar</p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50">
                ...
                <th className="text-left px-4 py-3 font-medium text-gray-700">Ubicacion</th>
