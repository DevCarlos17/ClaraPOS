...
- [ ] 4.1 **[BACKWARD-COMPAT GUARD]** Add/verify test asserting that when `moneda_presentacion_documentos` is absent (today's default state), all existing passing assertions for payments (`formatMontoPago` USD-native branch), `formatearCierre`, and the 2 final bold total rows remain byte-identical to pre-change output.
- [ ] 4.2 Run `yarn test:run` — all tests green, including extended `factura-export.test.ts`, `recibo-pagos.test.ts`, and new `company-data-form.test.tsx`.
- [ ] 4.3 Run `yarn type-check` — must be clean for non-test files. Spurious vitest-globals errors on `*.test.ts`/`*.test.tsx` files are EXPECTED noise, not a regression; use `yarn type-check:test` to validate test files instead.
