...
> **Correction note (post-verify)**: this requirement replaces the original "Conversion con Tasa
> Paralela Usa el Total" wording, which specified dividing `monto_usd` by the exchange rate for the
> BS/tasa-paralela case. That formula was implemented, caused a CRITICAL double-conversion bug
> caught by re-verification (a $116.00 total displayed as $2.90), and was corrected in commit
> `90c6673`. See `design.md`'s amendment for the root-cause trace. This is the final, verified spec.
