...
> **Coordination note**: ADDS bimonetary display only. Does NOT reorder totals rows (owned by
> `recibo-igtf-orden-abono-factura`) nor change payment grouping (owned by
> `recibo-desglose-pagos-orden`) — MODIFIED below only adds the missing USD counterpart to
> Bs-native payments.
...
del toggle del recibo. La suma en Bs MUST reconciliar con el total de factura en Bs.
(Previously: métodos Bs solo mostraban Bs, sin su equivalente USD.)
...
#### Scenario: Método Bs ahora también muestra su equivalente USD
